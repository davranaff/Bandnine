...
import { IeltsPageHeader, InsightListCard, MetricCard, IeltsStatusChip } from '../components';
...
export function AttemptResultView({ attemptId }: AttemptResultViewProps) {
  ...
  const resolvedAttemptId = attemptId || String(params.attemptId || '');

  const attemptQuery = useAttemptQuery(resolvedAttemptId);
  const resultQuery = useAttemptResultQuery(resolvedAttemptId);
  const integrityQuery = useAttemptIntegrityEventsQuery(resolvedAttemptId);
