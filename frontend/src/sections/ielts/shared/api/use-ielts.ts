...
import type { IeltsListFilters, MyTestsFilters, TeacherStudentsFilters } from './types';

const rootKey = ['ielts'];

export const ieltsQueryKeys = {
  root: rootKey,
  studentDashboard: [...rootKey, 'student-dashboard'],
  teacherDashboard: [...rootKey, 'teacher-dashboard'],
  moduleTests: (module: ActiveIeltsModule, filters: IeltsListFilters) =>
...
export function useStudentDashboardQuery() {
  return useFetch(ieltsQueryKeys.studentDashboard, getStudentDashboard);
}

export function useTeacherDashboardQuery() {
  return useFetch(ieltsQueryKeys.teacherDashboard, getTeacherDashboard);
}

export function useModuleTestsQuery(module: ActiveIeltsModule, filters: IeltsListFilters) {
  return useFetch(ieltsQueryKeys.moduleTests(module, filters), () =>
    getModuleTests(module, filters)
  );
}

export function useTestDetailsQuery(module: ActiveIeltsModule, testId: string) {
  return useFetch(ieltsQueryKeys.testDetails(module, testId), () => getTestById(module, testId), {
    enabled: Boolean(testId),
  });
}

export function useAttemptQuery(attemptId: string) {
  return useFetch(ieltsQueryKeys.attempt(attemptId), () => getAttemptById(attemptId), {
    enabled: Boolean(attemptId),
  });
}

export function useSessionQuery(attemptId: string) {
  return useFetch(ieltsQueryKeys.session(attemptId), () => getSessionData(attemptId), {
    enabled: Boolean(attemptId),
...
export function useAttemptResultQuery(attemptId: string) {
  return useFetch(ieltsQueryKeys.attemptResult(attemptId), () => getAttemptResult(attemptId), {
    enabled: Boolean(attemptId),
  });
}

export function useStudentAttemptsQuery(filters: MyTestsFilters) {
  return useFetch(ieltsQueryKeys.studentAttempts(filters), () => getStudentAttempts(filters));
}

export function useStudentProfileQuery() {
  return useFetch(ieltsQueryKeys.studentProfile, getStudentProfile);
}

export function useTeacherStudentsQuery(filters: TeacherStudentsFilters) {
  return useFetch(ieltsQueryKeys.teacherStudents(filters), () => getTeacherStudents(filters));
}

export function useTeacherStudentQuery(studentId: string) {
  return useFetch(
    ieltsQueryKeys.teacherStudent(studentId),
...
export function useTeacherAnalyticsQuery() {
  return useFetch(ieltsQueryKeys.teacherAnalytics, getTeacherAnalytics);
}

export function useAttemptIntegrityEventsQuery(attemptId: string) {
  return useFetch(
    ieltsQueryKeys.attemptIntegrity(attemptId),
...
function useInvalidateIeltsQueries() {
  const queryClient = useQueryClient();

  return () => {
    queryClient.invalidateQueries({ queryKey: ieltsQueryKeys.root });
  };
}

export function useStartAttemptMutation() {
  const invalidate = useInvalidateIeltsQueries();
